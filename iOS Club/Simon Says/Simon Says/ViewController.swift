...
//  Simon Says
//
//  Created by Abhinav Tirath on 2/27/18.
...
class ViewController: UIViewController {
    ...
    func playPattern(colorPosition: Int) {
        ...
        animateButton(buttonTag: pattern[colorPosition - 1], completionHandler: {
    ...
    func verifyPattern () {
        ...
        if (patternTag != playerTag) {
            print("You lose")
            resetGame()
        } else if patternIndex == currentLevel - 1 {
...
//        playPattern(colorPosition: 1)
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5, execute: {
            self.playPattern(colorPosition: 1)
        })
    }
